...
export default function Terms() {
  ...
  const handleAccept = async () => {
    setLoading(true);
    try {
      // For non-guest users, persist to backend
      if (user && !user.isGuest) {
        await api.put('/user/accept-terms');
      }
      acceptTerms();
      navigate('/dashboard', { replace: true });
    } catch (err) {
      // Even if backend fails, allow navigation for now
      acceptTerms();
      navigate('/dashboard', { replace: true });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 md:p-8 relative z-10">
      <div className="glass-panel p-6 md:p-10 max-w-2xl w-full relative">
        <div className="flex items-center gap-4 mb-8 border-b border-gray-800 pb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Please review before continuing</p>
          </div>
        </div>
        
        <div className="text-gray-400 space-y-4 mb-8 h-48 md:h-64 overflow-y-auto pr-4 custom-scrollbar">
          <p>
          ...
          <h3 className="text-lg text-white font-semibold mt-6 mb-2">1. Data Privacy</h3>
          <p>
          ...
          <h3 className="text-lg text-white font-semibold mt-6 mb-2">2. Usage Responsibilities</h3>
          <p>
          ...
          <h3 className="text-lg text-white font-semibold mt-6 mb-2">3. Rewards Program (Coming Soon)</h3>
          <p>
          ...
          <h3 className="text-lg text-white font-semibold mt-6 mb-2">4. Financial Disclaimer</h3>
          <p>
            Finance Flow is a student project designed for educational and tracking purposes. We do not provide investment advice or guarantee returns on savings.
          </p>
        ...
        <div className="bg-[#1a1c29] p-5 rounded-xl border border-gray-800 mb-6">
        ...
        >
          {loading ? 'Processing...' : (accepted ? 'Accept & Continue to Dashboard' : 'Accept Terms to Continue')}
        </button>
      </div>
    </div>
  );
